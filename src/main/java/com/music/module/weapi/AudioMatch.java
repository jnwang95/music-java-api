package com.music.module.weapi;

import cn.hutool.json.JSONObject;
import com.music.module.base.BaseModuleWeApi;
import org.springframework.stereotype.Service;

import java.util.Map;

/**
 * @author wjning
 */
@Service
public class AudioMatch implements BaseModuleWeApi {


    @Override
    public void execute(JSONObject object, Map<String, String> queryMap, Map<String, String> cookies) {
        cookies.put("os","pc");
        object.set("algorithmCode", "shazam_v2");
        object.set("times", 1);
        object.set("sessionId", "C999431ACDC84EDBB984763654E6F8D7");
        object.set("duration", 3.3066249999999995);
        object.set("from", "recognize-song");
        object.set("rawdata", "");
    }

    @Override
    public String getUrl() {
        return "https://music.163.com/api/music/audio/match" ;
    }
}
